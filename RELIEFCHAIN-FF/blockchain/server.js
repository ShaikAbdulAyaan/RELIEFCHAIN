...
async function handle(path, body) {
  if (path === "/blockchain/campaign") {
    const campaignId = requireString(body, "campaignId");
    const metadataHash = requireString(body, "metadataHash");
    const result = await import("./integration/blockchainAdapter.js").then(({ createCampaignProof }) =>
      createCampaignProof({ campaignId, metadataHash })
    );
    return standardResult(result);
  }

  if (path === "/blockchain/batch/transfer") {
    const batchId = requireString(body, "batchId");
    const toLocation = Number(body.toLocation);
    if (!Number.isInteger(toLocation) || toLocation < 1 || toLocation > 3) {
      throw new Error("toLocation must be 1=WAREHOUSE, 2=TRUCK, or 3=CAMP");
    }
    const result = await import("./integration/blockchainAdapter.js").then(({ transferReliefBatchProof }) =>
      transferReliefBatchProof({ batchId, toLocation })
    );
    return standardResult(result);
  }

...
export const transferBatch = async (req: Request, res: Response) => {
  try {
    ...
    const allowedTransitions: Record<BatchStatus, BatchStatus[]> = {
      [BatchStatus.CREATED]: [BatchStatus.VERIFIED, BatchStatus.DISPATCHED],
      [BatchStatus.VERIFIED]: [BatchStatus.DISPATCHED],
      [BatchStatus.DISPATCHED]: [BatchStatus.IN_TRANSIT],
      [BatchStatus.IN_TRANSIT]: [BatchStatus.RECEIVED],
      [BatchStatus.RECEIVED]: [BatchStatus.DISTRIBUTED],
    ...
    if (!allowedTransitions[currentStatus].includes(status)) {
      return res.status(400).json({
        success: false,
        message: `Invalid batch status transition: ${currentStatus} -> ${status}`,
      });
    }

    let blockchain = null;

    // The Solidity batch moves WAREHOUSE -> TRUCK -> CAMP.
    // VERIFIED is an off-chain verification state and does not move the batch.
    if (status === BatchStatus.DISPATCHED) {
      try {
        blockchain = await blockchainService.transferReliefBatch({
          batchId: batch.id,
          toLocation: 2,
        });

      } catch (blockchainError) {
        console.error("Batch dispatch blockchain proof failed:", blockchainError);
        return res.status(502).json({
          success: false,
          message: "Batch dispatch blockchain transaction failed",
        });
      }
    }
    ...
    if (status === BatchStatus.RECEIVED) {
      try {
        ...
        console.error("Batch arrival blockchain proof failed:", blockchainError);
        return res.status(502).json({
          success: false,
          message: "Batch arrival blockchain transaction failed",
        });
      }
    }
